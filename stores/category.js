  ...
  actions: {
    async create(formData) {
      ...
      const { data, error } = await useFetch(
        runtimeConfig.public.api_url + "/category/create",
        {
          method: "POST",
          body: formData,
          headers: {
    ...
    async getAllCategories(page, itemsPerPage) {
      ...
      const snackbar = useSnackbar();
      const { data, error } = await useFetch(
        runtimeConfig.public.api_url + "/category",
        {
          params: {
            page,
    ...
    async remove(id) {
      ...
      const token = localStorage.getItem("admin_auth_token");
      const { data, error } = await useFetch(
        runtimeConfig.public.api_url + "/category/" + id,
        {
          method: "DELETE",
          headers: {
            authorization: `Bearer ${token}`,
    ...
    async removeBulk(ids) {
      ...
      const { data, error } = await useFetch(
        runtimeConfig.public.api_url + "/category/delete-bulk",
        {
          method: "DELETE",
          body: { ids },
          headers: {
